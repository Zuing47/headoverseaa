...
export function HomeHero({ content, locale = "pt" }: HomeHeroProps) {
  ...
  return (
    <section className="relative min-h-[92svh] overflow-hidden bg-black text-white">
      ...
      />
      {/* Mobile: keep skyline readable — soft top + bottom only */}
      <div
        className="pointer-events-none absolute inset-0 lg:hidden"
        aria-hidden
        style={{
          background:
            "linear-gradient(180deg, rgba(5,5,5,0.35) 0%, rgba(5,5,5,0.12) 28%, rgba(5,5,5,0.2) 55%, rgba(5,5,5,0.78) 100%)",
        }}
      />
      <div
        className="pointer-events-none absolute inset-x-0 bottom-0 h-36 lg:h-48"
        aria-hidden
        style={{
          background:
            "linear-gradient(to top, #050505 0%, rgba(5,5,5,0.4) 50%, transparent 100%)",
        }}
      />

      <div className="relative z-10 grid min-h-[92svh] lg:grid-cols-12">
        <div className="relative flex flex-col justify-end px-[var(--page-padding-mobile)] pb-14 pt-24 md:px-[var(--page-padding-tablet)] lg:col-span-5 lg:justify-center lg:px-[var(--page-padding-desktop)] lg:pb-24 lg:pr-10 lg:pt-20">
          <motion.div
          ...
          <motion.h1
            ...
            className="font-display mt-7 max-w-[13ch] text-[clamp(3rem,6vw,5rem)] font-light leading-[1.02] tracking-[-0.03em] text-white"
          >
            {hero.title}
            {hero.titleAccent ? (
              <>
                {" "}
                <span className="text-white/70">{hero.titleAccent}</span>
              </>
            ) : null}
          </motion.h1>
          ...
          <motion.p
            ...
            className="mt-8 max-w-[34ch] text-[15px] leading-relaxed text-white/55 md:text-[16px]"
          >
            {hero.subtitle}
          </motion.p>
          ...
          {hero.trustLine ? (
            <motion.p
              ...
              className="mt-10 max-w-[38ch] text-[12px] leading-relaxed tracking-wide text-white/30"
            >
              {hero.trustLine}
            </motion.p>
          ) : null}
